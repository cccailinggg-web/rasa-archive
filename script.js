...
function getStock(){
  try{
    return {...DEFAULT_STOCK,...JSON.parse(localStorage.getItem(STORAGE_KEY))};
  }catch(e){
...
function updateStockUI(){
  const s=getStock();
  $('stockFlower').textContent=s.flower;
  $('stockSun').textContent=s.sun;
  $('stockNight').textContent=s.night;
}

function resetFlow(){
  step=0;
  scores={flower:0,sun:0,night:0};
  ranking=['flower','sun','night'];
...
function showDraw(){
  ...
  setTimeout(()=>{
    $('cardField').classList.add('shuffle');
  },420);
}
...
function chooseCard(){
  ...
  let chosen=ranking.find(k=>stock[k]>0);
...
function reveal(){
  ...
  $('resultEdition').textContent=card.edition;
